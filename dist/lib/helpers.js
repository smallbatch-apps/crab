...
export default () => {
    ...
    handlebars.registerHelper("propsDefinition", function (resourceName, options) {
        const { typeProps, extends: hasExtends, elementProps, elementType, justProps, } = options;
        if (justProps)
            resourceName = "";
        if (typeProps) {
            // Type syntax
    ...
    handlebars.registerHelper("refParams", function (options) {
        if (!options.forwardRef || options.javascript)
            return "";
    ...
    handlebars.registerHelper("reactImports", function (options) {
        ...
        const importString = !imports.length && options.importReact
            ? "React"
