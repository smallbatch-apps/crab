...
export const generateComponent = async (templateOptions: TemplateOptions) => {
  ...
  if (templateOptions.css) {
    templateOptions.imports.push(
      `import styles from './${templateOptions.resourceName}.module.css'`
    );
  }
  ...
  templateOptions.hasChildren = templateOptions.props.some(
    (p: PropState) => p.name === "children"
  );
...
const getPropName = (templateOptions: Record<string, any>): string => {
  if (templateOptions.javascript) return "";
  if (!templateOptions.props.length) return "";
...
function parseComponentPath(fullPath: string) {
